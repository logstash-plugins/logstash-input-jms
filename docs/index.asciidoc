...
----
 input
 {
    jms {
       ...
       include_header => false  <6>
       include_properties => false <6>
       include_body => true <6>
       use_jms_timestamp => false <7>
...
<6> Parts of the JMS Message to include in the event - headers, properties and the message body can be included or
    excluded from the event.
<7> Determines whether the JMSTimestamp header is used to populate the `@timestamp` field.
<8> Message selector: Use this to filter messages to be processed. The whole selector query should be double-quoted,
...
<1> Configuration settings. Note that here the `broker_url` definition includes a failover url. The syntax for the failover
    url is defined by your JMS provider, and you should consult the documentation for your JMS provider for the exact
    syntax.
